...
void main() {
  test('ported URLs still resolve to their Dart ripper', () {
    ...
    final readcomic = RipperFactory.getRipper(
      Uri.parse('https://read-comic.com/example-comic/'),
    );
    ...
    final scrolller = RipperFactory.getRipper(
      Uri.parse('https://scrolller.com/r/CatsStandingUp?filter=pictures'),
    );
    final shesFreaky = RipperFactory.getRipper(
      Uri.parse(
    ...
    final soundgasm = RipperFactory.getRipper(
      Uri.parse(
    ...
    final tapastic = RipperFactory.getRipper(
      Uri.parse('https://tapas.io/series/TPIAG'),
    );
    final teenplanet = RipperFactory.getRipper(
      Uri.parse(
        'http://teenplanet.org/galleries/the-perfect-side-of-me-6588.html',
      ),
    );
    final thechive = RipperFactory.getRipper(
      Uri.parse(
  ...
  test(
    ...
    () {
      final ripper = RipperFactory.getRipper(
        Uri.parse('https://www.twitch.tv/videos/123'),
      );
  ...
  test('migration catalog tracks feature parity progress', () {
    ...
    expect(RipperMigrationCatalog.portedRipperCount, 98);
    expect(RipperMigrationCatalog.unportedRipperCount, 18);
  });
}
