...
class TeenplanetRipper extends AbstractHTMLRipper {
  static final RegExp _gidPattern = RegExp(
    r'^.*teenplanet\.org/galleries/([a-zA-Z0-9\-]+)\.html$',
  );

  TeenplanetRipper(super.url);

  @override
  String getHost() => 'teenplanet';

  String getDomain() => 'teenplanet.org';
  ...
  static List<String> imageUrlsFromDocument(Document page) {
    return [
      for (final thumb in page.querySelectorAll('#galleryImages > a > img'))
        if (thumb.attributes.containsKey('src'))
          thumb.attributes['src']!.replaceAll('/thumbs/', '/'),
    ];
  }
