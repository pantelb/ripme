...
class ModelmayhemRipper extends AbstractHTMLRipper {
  static final RegExp _gidPattern = RegExp(
    r'^https?://www\.modelmayhem\.com/portfolio/(\d+)/viewall$',
  );

  static const Map<String, String> worksafeCookies = {'worksafe': '0'};

  ModelmayhemRipper(super.url);

  @override
  String getHost() => 'modelmayhem';

  String getDomain() => 'modelmayhem.com';
  ...
  Future<String> getGID(Uri url) async {
    ...
    throw FormatException(
      'Expected modelmayhem URL format: '
  ...
  static List<String> imageUrlsFromDocument(Document page) {
    final result = <String>[];
    for (final link in page.querySelectorAll('tr.a_pics > td > div > a')) {
      final src = link.querySelector('img')?.attributes['src'] ?? '';
      final imageUrl = src.replaceAll('_m', '');
